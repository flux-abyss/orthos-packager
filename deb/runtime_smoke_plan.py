...
def write_runtime_smoke_plan(meta: dict[str, Any], stage_dir: Path, orthos: Path) -> None:
    ...
    if bin_dir.is_dir():
        try:
            ...
            error(f"smoke: failed to read bin dir: {exc}; proceeding with partial/empty target list")

    # 2. Derive import targets from top_level.txt
    seen_imports = set()
    try:
        dist_infos = sorted(stage_dir.rglob("*.dist-info"))
    except OSError as exc:
        dist_infos = []
        error(f"smoke: failed to search for dist-info: {exc}; proceeding with partial/empty target list")
        ...
        if p.is_dir():
            ...
            if top_level.is_file():
                try:
                    ...
                    error(f"smoke: failed to read {top_level.name}: {exc}; skipping")
